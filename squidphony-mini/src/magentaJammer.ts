/**
 * Magenta Jammer — AI-generated melodic phrases that evolve alongside squid activity
 *
 * Architecture:
 *   1. Squid notes are collected into a rolling buffer
 *   2. Each cycle, a "squid phrase" is built from recent notes
 *   3. MusicVAE.interpolate() morphs between the LAST AI phrase and the new
 *      squid phrase — creating smooth musical evolution where the AI sounds
 *      like it's *listening and responding*
 *   4. Squid density controls behavior:
 *      - Few squids → similar() with high similarity (subtle, sparse)
 *      - Medium → interpolate() (responsive morphing)
 *      - Many squids → higher temperature, more adventurous
...
export class MagentaJammer {
    ...
    private squidCount = 0; // current squid density
    ...
    // ── Feed squid count for density-aware behavior ────────────
    feedActivity(count: number): void {
        this.squidCount = count;
    }
    ...
    private async maybeGenerate(): Promise<void> {
        ...
        try {
            ...
            const squidPhrase = this.buildSquidPhrase();
            let result: any;

            if (this.squidCount <= 3) {
                // ── FEW SQUIDS: subtle variations ──
                // Use similar() with high similarity — sparse, gentle
                const variations = await this.mvae.similar(squidPhrase, 1, 0.85, 0.8);
                result = variations[0];
                console.log(`[MagentaJammer] Mode: SUBTLE (${this.squidCount} squids, similarity=0.85)`);

            } else if (this.squidCount <= 7 && this.lastPhrase) {
                // ── MEDIUM SQUIDS: interpolate between last AI phrase and squid phrase ──
                // This creates smooth evolution — AI sounds like it's listening
                const interps = await this.mvae.interpolate(
                    [this.lastPhrase, squidPhrase], 4, 0.9
                );
                // Pick a point partway through the interpolation (not the endpoints)
                result = interps[2]; // 3rd of 4 = ~75% toward squid phrase
                console.log(`[MagentaJammer] Mode: INTERPOLATE (${this.squidCount} squids)`);

            } else {
                // ── MANY SQUIDS: adventurous ──
                // Use similar() with lower similarity + higher temperature
                const variations = await this.mvae.similar(squidPhrase, 1, 0.5, 1.4);
                result = variations[0];
                console.log(`[MagentaJammer] Mode: ADVENTUROUS (${this.squidCount} squids, similarity=0.5)`);
            }
    ...
    // ── Build a phrase from recent squid notes (with rhythmic variety) ──
    private buildSquidPhrase(): any {
    ...
    private playSequence(seq: any): void {
        ...
        for (const note of seq.notes) {
            ...
            // More squids = louder AI
            const densityBoost = Math.min(0.15, this.squidCount * 0.015);
        ...
        console.log(`[MagentaJammer] 🎵 Playing phrase: ${noteCount} notes (squids: ${this.squidCount})`);
    }
